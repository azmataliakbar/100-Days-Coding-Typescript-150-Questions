...
interface Album { // interface is used for creating object , artist/title/tracks are properties of object.
  artist: string;
  title: string;
  tracks?: number;
}

function make_album(artist: string, title: string, tracks?: number): Album { // tracks? means track number is optional.
  let album: Album = {
      artist: artist,
      title: title
  };
...
let album1: Album = make_album("Artist1", "Album1", 10);
let album2: Album = make_album("Artist2", "Album2");
let album3: Album = make_album("Artist3", "Album3", 15);
